...
public class GetArticlesController {
    ...
    @RequestMapping(value="user/get_articles", method=RequestMethod.GET)
    public String getMovies() {
        return "this controller is for getting movie articles...";
    }
    
    @RequestMapping(value="user/get_articles", method=RequestMethod.POST)
    public String getMovies(@RequestBody JSONObject jsonObj) {
    	...
    	try {
    		if (jsonObj.containsKey(Keyword.USER_ID)) {
				...
				String SQL = String.format("select id from movie_article where author_id = %d", jsonObj.getInt(Keyword.USER_ID));
				if (jsonObj.containsKey(Keyword.ARTICLE_ID)) SQL += String.format(" and id < %d", jsonObj.getInt(Keyword.ARTICLE_ID));
